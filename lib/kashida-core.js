...
let babelParser, babelTraverse, postcss, scssSyntax, lessSyntax;
...
// SCSS/LESS carry syntax the default CSS parser rejects (nesting, $vars, // and
// `//` comments, mixins). Without the right parser postcss.parse() throws and the
// file silently passes as clean — a dangerous false-clean. Use the dialect parser
// when present; degrade to a silent skip only if it's genuinely missing.
try { scssSyntax = require('postcss-scss'); } catch { scssSyntax = null; }
try { lessSyntax = require('postcss-less'); } catch { lessSyntax = null; }
...
// `font` shorthand → pull the size, optional /line-height, and the family list so
// small-size / tight-leading / latin-font-stack fire on it too (a stack packed
// into `font:` was previously invisible). Returns null for the system-font
// keywords (caption/icon/menu…) and CSS-wide values, which carry no size/family.
const FONT_SIZE_TOK = '(?:-?\\d*\\.?\\d+(?:px|pt|pc|em|rem|ex|ch|%|cm|mm|in|q|vw|vh|vmin|vmax)|xx-small|x-small|small|medium|large|x-large|xx-large|smaller|larger)';
const FONT_SHORTHAND_RE = new RegExp('(?:^|\\s)(' + FONT_SIZE_TOK + ')(?:\\s*/\\s*(\\S+?))?\\s+(\\S.*)$', 'i');
function parseFontShorthand(value) {
  const v = String(value).trim().replace(/\s*!important\s*$/i, '');
  if (/^(?:caption|icon|menu|message-box|small-caption|status-bar|inherit|initial|unset|revert(?:-layer)?)$/i.test(v)) return null;
  const m = FONT_SHORTHAND_RE.exec(v);
  if (!m) return null;
  return { size: m[1], lh: m[2] || null, family: m[3].trim() };
}
...
function selectorScripts(selector) {
  ...
  if (/\.(?:ar|arabic|rtl)\b/.test(selector)) out.push(BY_NAME.get('Arabic'));
...
// One CSS declaration → findings. Shared by the CSS scan (rule selectors supply
// the script context) and inline style="" attributes in markup (the element's
// lang/dir/nearby text supply it). Vendor prefixes are stripped so
// `-webkit-font-feature-settings` is checked too.
function checkDecl(prop, rawValue, scripts, line, push, ctx) {
  if (!scripts.length) return;
  prop = String(prop).toLowerCase().replace(/^-(?:webkit|moz|ms|o)-/, '');
  ...
  else if (prop === 'font-family') { if (fontStackNeedsArabic(rawValue, ctx.extraArabic) && fontFires(scripts)) push('latin-font-stack', line, v, MSG.font); }
  else if (prop === 'font') {
    // The `font` shorthand packs size[/line-height] and the family list together.
    const fs = parseFontShorthand(rawValue);
    if (fs) {
      const from = `font: ${v}`;
      if (fs.size && smallLen(fs.size) && sizeFires(scripts)) push('small-size', line, from, MSG.size);
      if (fs.lh && tightLH(fs.lh) && leadingFires(scripts)) push('tight-leading', line, from, MSG.leading);
      if (fs.family && fontStackNeedsArabic(fs.family, ctx.extraArabic) && fontFires(scripts)) push('latin-font-stack', line, from, MSG.font);
    }
  }
}
...
function scanCss(file, src, findings, ctx) {
  ...
  const syntax = ext === '.scss' ? scssSyntax : ext === '.less' ? lessSyntax : null;
  let root;
  try { root = (syntax || postcss).parse(src); } catch { return; }
  const push = (rule, line, from, msg) => findings.push({ rule, sev: 'flag', line, from, to: '', msg });
  root.walkDecls(decl => {
    const scripts = ruleScripts(decl);
    if (!scripts.length) return;
    const line = decl.source && decl.source.start ? decl.source.start.line : 1;
    checkDecl(decl.prop, decl.value, scripts, line, push, ctx);
  });
}

// Iterate the declarations of an inline style="" value. Uses PostCSS when present
// (robust to url()/colons in values); otherwise a conservative split.
function eachInlineDecl(styleText, cb) {
  if (postcss) {
    let root; try { root = postcss.parse(styleText); } catch { return; }
    root.walkDecls(d => cb(d.prop, d.value));
  } else {
    for (const part of String(styleText).split(';')) {
      const i = part.indexOf(':'); if (i < 0) continue;
      cb(part.slice(0, i).trim(), part.slice(i + 1).trim());
    }
  }
}
...
// The script context for the element that owns an attribute at `attrIndex`:
// the file-level <html lang/dir>, plus the tag's own lang/dir, plus the text
// immediately following the tag (up to the next child element).
function elementScripts(src, attrIndex, htmlScripts) {
  const tagStart = src.lastIndexOf('<', attrIndex);
  const gt = src.indexOf('>', attrIndex);
...
const MARKUP_STYLE_RE = /\bstyle\s*=\s*("([^"]*)"|'([^']*)')/gi;
function scanMarkup(file, src, findings, ctx) {
  ...
  while ((m = MARKUP_CLASS_RE.exec(src)) !== null) {
    ...
    const scripts = elementScripts(src, m.index, htmlScripts);
  ...
  // Inline style="" declarations (letter-spacing / font-family / font shorthand /
  // …) on an element with an Arabic-script context. Skip templated values.
  MARKUP_STYLE_RE.lastIndex = 0;
  while ((m = MARKUP_STYLE_RE.exec(src)) !== null) {
    const styleText = m[2] !== undefined ? m[2] : m[3];
    if (!styleText || /[{}$]/.test(styleText)) continue;
    const scripts = elementScripts(src, m.index, htmlScripts);
    if (!scripts.length) continue;
    const line = lineAt(src, m.index);
    eachInlineDecl(styleText, (prop, value) => checkDecl(prop, value, scripts, line, push, ctx));
  }
}
...
function scanSource(file, src, opts = {}) {
  ...
  if (JS_EXT.has(ext)) { scanJs(file, src, ext, findings, ctx); scanCursiveSplit(src, findings); }
    ...
    scanMarkup(file, src, findings, ctx);
    // Embedded <style> blocks (esp. Vue/Svelte/Astro SFCs, where most CSS lives)
    // get full CSS coverage — run scanCss on each block and offset the line
    // numbers back to the file.
    const STYLE_BLOCK_RE = /<style\b[^>]*>([\s\S]*?)<\/style>/gi;
    let sm;
    while ((sm = STYLE_BLOCK_RE.exec(src)) !== null) {
      const inner = sm[1];
      if (!inner.trim()) continue;
      const offset = lineAt(src, sm.index + sm[0].indexOf(inner)) - 1;
      const sub = [];
      scanCss(file, inner, sub, ctx);
      for (const f of sub) { f.line += offset; findings.push(f); }
    }
    scanCursiveSplit(src, findings);
  }
