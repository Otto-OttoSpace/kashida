...
// SSRF guard: block loopback / private / link-local / cloud-metadata targets so
// `kashida --url` (and the linked-CSS sub-fetches it makes) can't be pointed at
// internal services.
function isPrivateIp(ip) {
  ...
  if (net.isIPv6(ip)) {
    ...
    return l === '::1' || l === '::' || l.startsWith('fc') || l.startsWith('fd') || l.startsWith('fe80') ||
      l.startsWith('::ffff:127.') || l.startsWith('::ffff:10.') || l.startsWith('::ffff:169.254') || l.startsWith('::ffff:192.168') || /^::ffff:172\.(1[6-9]|2\d|3[01])\./.test(l);
  }
  return false;
}
function assertPublicHost(hostname) {
  return new Promise((resolve, reject) => {
    ...
    if (h === 'localhost' || h.endsWith('.localhost') || h.endsWith('.local') || h === 'metadata.google.internal') return reject(new Error('blocked host: ' + hostname));
    if (net.isIP(h)) return isPrivateIp(h) ? reject(new Error('blocked private IP: ' + hostname)) : resolve();
    dns.lookup(h, (err, address) => {
      if (err) return reject(err);
      if (isPrivateIp(address)) return reject(new Error('host resolves to a private IP: ' + address));
      resolve();
    });
  });
}
...
async function run(url, opts = {}) {
  // SSRF guard on the primary (Playwright) path too — `run()` is the default for
  // `--url`; without this it would page.goto() straight to an internal target.
  const u = new URL(url);
...
function fetchText(url, { binary = false, timeout = 20000, redirects = 0, maxBytes = 10 * 1024 * 1024 } = {}) {
  return new Promise((resolve, reject) => {
    ...
    assertPublicHost(u.hostname).then(() => {
      ...
      const req = lib.get(url, { headers: { 'User-Agent': 'kashida/0.4 (+https://kashida.ottospace.co)' } }, (res) => {
        if (res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
          ...
          return resolve(fetchText(next, { binary, timeout, redirects: redirects + 1, maxBytes }));
        }
        const chunks = [];
        let total = 0;
        res.on('data', c => {
          total += c.length;
          if (total > maxBytes) { req.destroy(); res.destroy(); return reject(new Error('response exceeds ' + maxBytes + ' bytes')); }
          chunks.push(c);
        });
